# Ticket — `chore/rename-alcove`: rebrand "Runout" → "Alcove"

**For:** Front End Developer (implement) · **Owner:** Marketing Manager
**Branch:** `chore/rename-alcove` (off `main`) → PR to `main` — **never commit to `main`**
**Copy source:** `marketing/copy-kit-alcove.md` (§2–§3)

> Decision, rationale, and open `[VALIDATE]` items live in
> `marketing/rename-alcove.md`. Scope below is exact.
...
| --- | --- | --- | --- | --- |
| 1 | `src/components/Header.jsx` | 52 | `<span className="wordmark">Runout</span>` | `<span className="wordmark">Alcove</span>` |
| 2 | `src/AuthScreen.jsx` | 48, 66, 99, 138 | `<h1 className="auth-wordmark">Runout</h1>` (×4) | `Alcove` |
| 3 | `src/App.jsx` | 46 | `<h1 className="auth-wordmark">Runout</h1>` | `Alcove` |
| 4 | `src/__tests__/header-toolbar.test.jsx` | 29 | `screen.getByText('Runout')` | `screen.getByText('Alcove')` |
| 5 | `index.html` | 9 | `apple-mobile-web-app-title` = `Runout` | `Alcove` |
| 6 | `index.html` | 16 | `<title>Runout — Records &amp; Books</title>` | `<title>Alcove — Records &amp; Books</title>` |
| 7 | `index.html` | 10 | current `meta description` | use `copy-kit-alcove.md` §3 short description |
| 8 | `vite.config.js` | 12 | `name: 'Runout — Records & Books'` | `name: 'Alcove — Records & Books'` |
| 9 | `vite.config.js` | 13 | `short_name: 'Runout'` | `short_name: 'Alcove'` |
| 10 | `vite.config.js` | 14 | current `description` | use `copy-kit-alcove.md` §3 |
| 11 | `public/*` | — | `favicon.png`, `apple-touch-icon.png`, `icon-*.png` | **new mark — GATED on design `[VALIDATE]`** (see §5) |
| 12 | `README.md` | 1–67 | brand text ("Runout is a progressive web app…") | brand text → "Alcove"; **keep all `RUNOUT_ADMIN_KEY` / `runout-collection` / `runout-library` / `RU-…` identifiers** |
| 13 | `docs/functional.md`, `docs/technical.md`, `docs/design-redesign.md` | — | brand references | brand references → "Alcove"; **keep technical identifiers** |
...
- Short description / meta / PWA description → `copy-kit-alcove.md` §3
  (the ≤160-char line).
...
- Rename `--runout-gold` → `--alcove-gold` across `src/` CSS (touches ~15
...
- **Icons `[VALIDATE]`** — new mark on the dark `#16130F` theme (Figma / UI/UX
  Expert). Do not ship the stale Runout icon with the Alcove name. Meanwhile,
  the code + manifest rename can land on the branch.
- **Native-speaker + legal checks `[VALIDATE]`** — `marketing/name-check-alcove-7-languages.md`
  and `rename-alcove.md` §5. None block the code change; all block the public
  launch ("name day").
...
2. `npm run lint`
3. `npm test` (the `header-toolbar.test.jsx` assertion must pass with "Alcove")
...
- [ ] Wordmark = `Alcove` in `Header.jsx`, `AuthScreen.jsx` (×4), `App.jsx`
...
- [ ] On `chore/rename-alcove`, PR opened to `main`, not pushed directly
