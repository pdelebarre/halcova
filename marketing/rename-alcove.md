...
**Rename the public brand from `Runout` to `Halcova`.**
...
**Why Halcova:** a warm recessed nook that holds your treasures — and it is a
real, natural word in the **same family across all seven target languages**
(`Halcova` / `alcôve` / `alcova` / `alkoof` / `Alkoven` / `alcoba` / `alcova`).
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Halcova | alcôve | alcova | alkoof | Alkoven | alcoba | alcova | none found |
...
1. **Trademark search** — EUIPO, USPTO, WIPO, INPI, and Japan/other target
   ...
   *Halcova* coworking and any "Halcova" apps.
2. **Domain check** — exact-match `.com`, `.app`, plus country TLDs for target
   markets; expect `getHalcova.*` / `tryHalcova.*` / `.app` as realistic homes.
...
4. **Social handles** — `@Halcova` on Instagram/TikTok/Bluesky/YouTube/X.
...
| --- | --- | --- | --- | --- |
| 1 | In-app header wordmark | `src/components/Header.jsx` (~line 52, `<span className="wordmark">`) | `Runout` | `Halcova` |
| 2 | Auth "no collections" screen | `src/App.jsx` (~line 46, `auth-wordmark`) | `Runout` | `Halcova` |
| 3 | Browser tab title | `index.html` (`<title>`) | `Runout — Records & Books` | `Halcova — Records & Books` |
| 4 | iOS home-screen title | `index.html` (`apple-mobile-web-app-title`) | `Runout` | `Halcova` |
| 5 | SEO description | `index.html` (`meta description`) | current | repositioned line (§3) |
| 6 | PWA install name | `vite.config.js` (`manifest.name`) | `Runout — Records & Books` | `Halcova — Records & Books` |
| 7 | PWA short name (icon label) | `vite.config.js` (`manifest.short_name`) | `Runout` | `Halcova` |
...
| 10 | README title & copy | `README.md` | "Runout — …" | "Halcova — …" |
| 11 | Docs / instructions (brand refs) | `docs/functional.md`, `docs/technical.md`, `docs/design-redesign.md`, `.github/copilot-instructions.md` | Runout | Halcova |
...
| Brand recall | **branded search %** (e.g. "Halcova app") | the rename KPI — do people search the name? |
...
4. **Implement** items 1–8 of §6 on a `chore/rename-Halcova` branch (never `main`).
5. **Name day** — flip the PWA manifest + wordmark + icons together so installed
   home-screen icons don't fork.
6. **Launch copy** — `copy-kit-Halcova.md`.
