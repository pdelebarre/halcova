# Runout — Technical Documentation

This document describes **how Runout is built**: architecture, data model, APIs,
...
Runout is a **React 19 SPA** built with Vite, deployed as a **static site + a
...
flowchart LR
    subgraph Browser [PWA in the browser]
        ...
        LS[(localStorage<br/>session + per-user Discogs token)]
    ...
    subgraph Netlify [Netlify]
        NF[Netlify Functions<br/>/collection /auth /admin]
        BLOB[(Netlify Blobs<br/>runout-identity,<br/>collection-&lt;userId&gt;-&lt;kind&gt;,<br/>owner stores)]
        NF --> BLOB
    end

    UI -->|Bearer access code| NF
    UI -->|fetch| D[Discogs API]
    UI -->|fetch| G[Google Books API]
```

Two boundaries matter:

1. **Lookup APIs (Discogs, Google Books)** are called **directly from the
   browser**. The Discogs token never touches Netlify; it lives only in
   `localStorage` (per user) and is sent only to Discogs.
...
└── src/
    ...
    ├── api/
    │   ├── discogs.js             # Discogs search + release detail client (per-user token)
    │   ├── books.js               # Google Books client (ISBN/text/detail)
    ...
    └── components/
        ...
        └── SettingsModal.jsx      # Discogs token management
```
...
The "server" is **three Netlify Functions** sharing helper modules, all backed
...
- **`api/discogs.js`** — builds URLs against `https://api.discogs.com`,
  appends the `token` query param from `localStorage`, sets a `User-Agent`
  (`RunoutRecordCollector/1.0`, required by Discogs), and maps
  `401 → BAD_TOKEN`, `429 → RATE_LIMIT`. Normalizes search results and release
  details (tracklist, notes, images). `parseFormatType` infers a coarse
  `LP/EP/CD/7"/12"/Other` from the raw format array.
- **`api/books.js`** — calls the public Google Books `v1` endpoints with no
  auth, normalizes volumes into the shared item shape, upgrades thumbnail URLs
...
- **Discogs token** — `discogs.hasToken/setToken/clearToken` wrap a
  `localStorage` key **namespaced per user** (`runout_discogs_token_<userId>`),
  so each member pastes their own token and switching accounts never leaks one.
- `ScannerModal` is loaded with `React.lazy` + `Suspense` so the ~heavy WASM
  decoder only downloads when the user actually taps **Scan**.
- The scanner is also guarded so that if no token exists, the app routes to
...
- **Manifest** — name "Runout — Records & Books", standalone, portrait,
...
- **Runtime caching**:
  - `api.discogs.com` → `NetworkFirst`, 200-entry cache (`discogs-api`).
  - Discogs images → `CacheFirst`, 500 entries, 30-day expiry.
  - `www.googleapis.com` (books API) → `NetworkFirst`, 200 entries.
  - `books.google.com` images → `CacheFirst`, 500 entries, 30-day expiry.
...
`black-translucent` status bar, `Runout` title) and `theme-color`.
...
- Auth: `token` query parameter (personal access token), plus a required
  `User-Agent`.
- Rate limit: Discogs enforces ~60 req/min unauthenticated behavior; the client
  maps `429` to a friendly `RATE_LIMIT` error.
...
- Auth: none (public), `country=US` param for region-appropriate results.
...
- The **Discogs token never leaves the browser** — it is sent only to
  `api.discogs.com`; Netlify is not involved in lookups and never sees it.
- The token is stored per-user in `localStorage` on-device; the Settings modal
  offers a one-tap **Remove token**.
