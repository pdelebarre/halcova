# Hokan — your crate and shelf, cataloged

**Scan a barcode, catalog the thing.** Hokan is a progressive web app (PWA) for
cataloging your record and book collections. Point your phone's camera at a
barcode, Hokan looks the release up on **Discogs** (records) or **Google Books**
...
Hokan supports **multiple users**: you (the site owner) approve who gets in,
...
- **Duplicate detection** — before anything is added, Hokan tells you whether
...
2. **Confirm**: if there's a single match you land straight on the result; if
   several pressings/editions match, pick the right one. Hokan flags anything
...
Hokan has no passwords — access is granted by you. The flow is:
...
2. **You approve from the admin panel** — signed in with your admin key, tap the
   ...
   **Books**) and Hokan generates an `RU-XXXX-XXXX-XXXX` access code for you to
   share.
