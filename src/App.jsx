...
export default function App() {
  const { items, status, error, add, update, remove, refresh } = useCollection()

  const [modal, setModal] = useState(null) // 'scan' | 'pick' | 'manual' | 'result' | 'detail' | 'settings'
  ...
  // Distinct genres and artists present in the crate — drives the classification filters.
  ...
  // (barcode auto-match, picking from multiple pressings, text search, manual
  // entry) funnels through here before anything gets added.
  ...
  async function handleBarcodeDetected(barcode) {
    ...
    try {
      const results = await discogs.searchByBarcode(clean)
      if (results.length === 1) {
      ...
      if (err.code === 'NO_TOKEN') {
        setModal('settings')
        showToast('Add a Discogs token first to look up barcodes')
        return
      }
  ...
  return (
    <>
      <Header onOpenSettings={() => setModal('settings')} />
      ...
      <main className="app-main">
        {status === 'loading' && <p className="status-line">Loading your crate…</p>}
        ...
        {status === 'ready' && items.length === 0 && (
          <EmptyState kind="empty" onScan={() => setModal('scan')} />
        )}
        ...
        {status === 'ready' && visibleItems.length > 0 && (
          <AlbumGrid items={visibleItems} onOpen={(item) => { setSelectedItem(item); setModal('detail') }} />
        )}
      </main>
      ...
      {modal === 'manual' && (
        <ManualAddModal onPick={presentCandidate} onClose={() => setModal(null)} />
      )}
