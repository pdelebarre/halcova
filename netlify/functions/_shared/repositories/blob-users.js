// Blobs-backed identity repository (ADR-0002 Phase 0/1). This is the CURRENT
// runout-identity blob implementation, kept verbatim so the existing Blobs path
// behaves byte-for-byte as before. `users.js` is now a thin facade over the
// repository interface: when DATABASE_URL is unset these functions run exactly
// as they always have; when it is set the Postgres repo is the primary and
// these become the read-through fallback / dual-write mirror.
//
