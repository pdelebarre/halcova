...
//
// Direct unit tests for the Blobs AI provider-profile store
// (ai-config-blob.js, ADMIN-3.2 #304) with an injected in-memory store.
// Complements ai-admin.test.js by exercising the store surface directly so the
// guard/edge branches (junk ids, partial updates, empty lists, missing target)
// are covered, not just the happy path the facade drives.

import { beforeEach, describe, expect, it } from 'vitest'
import { createAiConfigBlobStore } from './ai-config-blob'

// In-memory @netlify/blobs-shaped store.
...
beforeEach(() => {
  store = createStore()
  repo = createAiConfigBlobStore({ store })
})
...
describe('updateProfile', () => {
  ...
  it('returns null when the id is unknown', async () => {
    expect(await repo.updateProfile('00000000-0000-0000-0000-000000000099', { name: 'X' })).toBeNull()
  })
})
...
describe('transaction', () => {
  it('runs the op directly against the same store (no real transactions)', async () => {
