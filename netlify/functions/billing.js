...
// SEC-7.4 (#341): a HARD per-IP 429 throttle for INVALID-SIGNATURE bursts, in
// addition to the anomaly signal below. Stripe only ever sends valid signatures,
// so a flood of bad signatures from one IP is a forged-event probe / replay
// attack — throttling it hard bounds the CPU + vertex cost of verifying junk.
const WEBHOOK_INVALID_SIG_IP_LIMIT = Number(process.env.RUNOUT_WEBHOOK_INVALID_SIG_RATE_LIMIT) || 20
...
export default async (req) => {
  ...
  if (!verifyWebhookSignature({ rawBody, signature })) {
    ...
    // SEC-7.4 (#341): hard per-IP 429 throttle on top of the anomaly — a forged
